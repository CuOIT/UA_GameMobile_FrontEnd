...
let md = "# Báo Cáo Tổng Hợp Các Visual Dành Cho Gemini Web (Lesson 1 - 30)\n\n";
md += "> [!IMPORTANT]\n";
md += "> Vui lòng push code lên GitHub trước (`git push origin main`) để các hình ảnh được cập nhật online.\n\n";
md += "| STT | Lesson | ID | Origin (Placeholder) | Visual |\n";
md += "|---|---|---|---|---|\n";
...
for (const row of rows) {
    if (row.day !== currentLesson) {
        currentLesson = row.day;
        lessonStt = 1;
    }
    const id = `${row.day}.${lessonStt}`;
    
    let finalVisual = row.visual;
    if (row.day >= 25 || (row.day === 24 && finalVisual.includes('data-visual'))) {
        finalVisual = finalVisual.replace('.png', '.svg');
    }

    md += `| ${stt} | ${row.day} | ${id} | ${row.placeholder} | ![${row.placeholder}](${githubRawBase}/${finalVisual}) |\n`;
    stt++;
    lessonStt++;
}

import fs from 'fs';
fs.writeFileSync('C:\\Users\\DPC00212\\.gemini\\antigravity\\brain\\c2fc37b7-dd92-4bfc-b7a2-50bc818843ba\\gemini_web_visual_report.md', md, 'utf8');
